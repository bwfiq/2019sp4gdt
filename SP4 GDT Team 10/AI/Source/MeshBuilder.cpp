...
Mesh * MeshBuilder::GenerateTriangle(const std::string & meshName, Color color, float length)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	v.pos.Set(-length * 0.5f, -length * sin(Math::DegreeToRadian(60)) * 0.5f, 0);
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(length * 0.5f, -length * sin(Math::DegreeToRadian(60)) * 0.5f, 0);
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(0, length * sin(Math::DegreeToRadian(60)) * 0.5f, 0);
	v.color = color;
	vertex_buffer_data.push_back(v);
...
Mesh * MeshBuilder::GenerateCross(const std::string & meshName, Color color, float length)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	v.pos.Set(length * 0.5f, 0, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	vertex_buffer_data.push_back(v);
	v.pos.Set(-length * 0.5f, 0, 0);
